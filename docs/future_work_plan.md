
Roadmap
## Update project board UI/UX and allow for manual task insertion or quote tracking [FINISHED]
## Finish adding and completing Stripe Payment (seat-based-pricing branch) [FINISHED]
## Add ability to add files to a proposal for storage of information [FINISHED]
## Fix form builder determining whether to have set pricing section (whether via form or not) [FINISHED]

## Add in Forms so we can convert to pure company agnostic, form based quotes -> Need to complete full scale design of forms [use images to help via the design] (form-builder-system) [FINISHED]

## Add in Templates for customizable pdf downloads -> Need to add full referencing off templates [IN-PROGRESS]

## Refactor Proposals table to support new generalized form-builder and template generator [FINISHED]
## Update Analytics to Support New Proposals Table [FINISHED]

## Product specifications & how that relates to products catalog 

## GENERAL UI/UX Updated Design


## Add service management for recurring billing services (like service contracts or, software services to keep track of what’s getting charged to you) and then you could have recurring or flat rate 

## Add a calender to be able to input events (links to reminders)

## Add notifications for each users

## Add an approval process for sending quotes, directly via the email and to customize the email…

## Add integrations for quickbooks and sending invoices

## Need to figure out how to handle data ingestion from other platforms

## Emails ingestion AI insight about what stuff to do


