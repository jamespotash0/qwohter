    ...
    const wallDescriptions = wallEntries.map(([wallName, wall]) => {
      ...
      if (isGlassWall(wall)) {
        ...
        return SmartQuoteHelper.buildSentence([
          ...
          { text: `Panels are finished in <strong>${panelFinishCategoryMarked}${wall.panelFinishSpecificItem && wall.panelFinishSpecificItem !== 'Unknown' ? ` - ${panelFinishSpecificItemMarked}` : ''}</strong> (from the manufacturer's standard offerings)`, condition: SmartQuoteHelper.hasValue(wall.panelFinishCategory) },
