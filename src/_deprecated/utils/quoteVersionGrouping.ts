/**
 * Quote Version Grouping Utilities
 *
 * @deprecated Use proposalVersionGrouping.ts instead. This file is only kept
 * for backward compatibility with deprecated quote-based components.
 *
 * Groups quotes by their base proposal number for display purposes.
 * Example: P1001, P1001.1, P1001.2 all group under "P1001"
 */

import type { Quote } from '@/_deprecated/services/quotesService';
import type { QuoteStatus } from '@/_deprecated/lib/types/quotes/quote';
...
/**
 * Sort quotes by version number
 * Base (P1001) comes first, then versions in order (.1, .2, .3)
 */
export function sortQuotesByVersion(quotes: Quote[]): Quote[] {
...
/**
 * Group quotes by their base proposal number
 */
export function groupQuotesByVersion(quotes: Quote[]): QuoteVersionGroup[] {
  ...
  quotes.forEach(quote => {
    ...
    grouped.get(baseNumber)!.push(quote);
  });

  // Convert to QuoteVersionGroup[]
 ...
 */
export function getVersionStatusBadges(group: QuoteVersionGroup) {
  const badges: Array<{
    status: QuoteStatus;
    count: number;
    label: string;
...
/**
 * Check if a quote is the latest version in its group
 */
export function isLatestVersion(quote: Quote, allQuotes: Quote[]): boolean {
  const baseNumber = getBaseProposalNumber(quote.proposal_number);
  const versionsInGroup = allQuotes.filter(
    q => getBaseProposalNumber(q.proposal_number) === baseNumber
  );
  const sorted = sortQuotesByVersion(versionsInGroup);
  return sorted[sorted.length - 1]!.id === quote.id;
}

/**
 * Get quote version number (1, 2, 3, etc.) from proposal number
 * "P1001" -> 1 (base version)
 * "P1001.1" -> 2
 * "P1001.2" -> 3
 */
export function getQuoteVersionNumber(proposalNumber: string): number {
