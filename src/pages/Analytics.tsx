...
const Analytics = () => {
  ...
  // Fetch proposals using React Query (includes automatic realtime subscriptions)
  ...
  return (
    ...
    >
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <KPICard
              ...
              value={formatCurrency(mainVersionProposals.filter(q => q.proposal_status === 'Accepted').reduce((sum, q) => sum + (q.total_value || 0), 0))}
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={analytics.productModelMetrics} layout="vertical" margin={{ left: -10, right: 10, top: 5, bottom: 5 }}>
                    ...
                    <Tooltip
                      ...
                      formatter={(value: number, name: string) => {
                        if (name === 'Proposals') return [value, 'Quote Count'];
                        return [value, name];
                      }}
                      labelFormatter={(label, payload) => {
                      ...
                      <PieChart>
                        <Pie
                          data={[
                            ...
                            { name: 'Accepted', value: analytics.statusBreakdown.accepted, color: '#10B981' },
                        ...
                        <Legend
                          ...
                          formatter={(value, entry: any) => {
                            const total = analytics.statusBreakdown.incomplete +
                                         ...
                                         analytics.statusBreakdown.accepted +
      ...
      {/* Enlarged Chart Modal - Placeholder for future implementation */}
      {enlargedChart && (
        <Dialog open={!!enlargedChart} onOpenChange={(open) => !open && setEnlargedChart(null)}>
          <DialogContent className="max-w-[95vw] h-[92vh] p-3 pb-1 flex flex-col overflow-hidden">
            <div className="flex-1 flex items-center justify-center">
              <p className="text-gray-500">Enlarged chart view: {enlargedChart}</p>
            </div>
          </DialogContent>
        </Dialog>
      )}
