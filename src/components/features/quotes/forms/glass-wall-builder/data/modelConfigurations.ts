import type { ModelConfigurations } from '../types';

export const MODEL_CONFIGURATIONS: ModelConfigurations = {
  Stella: {
...
export const getFrameThickness = (model: string, stcRating?: string): string => {
  switch (model) {
    case 'Stella':
...
export const getPanelWidth = (model: string): string => {
  switch (model) {
    case 'Stella':
      return '51"';
    case 'Luna':
