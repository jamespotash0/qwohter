...
interface SignaturePositions {
  signature?: DetectedPosition;
  date?: DetectedPosition;
}

/**
 * Compute SHA-256 hash of PDF bytes for tamper detection
 */
async function computePdfHash(pdfBytes: Uint8Array): Promise<string> {
...
/**
 * Scan PDF for signature placement markers using pdfjs-dist
 * Looks for "Signature:" with underscores (from {{SIGNATURE_BLOCK}} replacement)
 * Returns detected positions or empty object if not found
 */
async function scanPdfForSignatureMarkers(pdfBytes: Uint8Array): Promise<SignaturePositions> {
  const positions: SignaturePositions = {};

  try {
    // Dynamic import of pdfjs-dist legacy build (works in Deno without web worker)
    //@ts-ignore
    ...
    for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
      ...
      for (let i = 0; i < textContent.items.length; i++) {
        ...
        // Look for the "Signature: ____" pattern generated by generateSignaturePlaceholder()
        if (text.includes('Signature') && text.includes('____')) {
          ...
          // Check if the date is part of the same text item
          if (text.includes('Date') && text.includes('____')) {
            const dateIdx = text.indexOf('Date');
            const textWidth = item.width || 400;
            const approxCharWidth = textWidth / text.length;
            positions.date = {
              pageIndex: pageNum - 1,
              x: x + (dateIdx * approxCharWidth),
              y,
              width: 150,
              height: item.height || 20,
            };
          ...
          break; // Found signature marker
        }
      }

      // Also check for Date on a separate text item if not found yet
      if (positions.signature && !positions.date) {
        for (let i = 0; i < textContent.items.length; i++) {
          const item = textContent.items[i];
          if (!('str' in item) || !item.str) continue;
          if (item.str.includes('Date') && item.str.includes('____')) {
            positions.date = {
              pageIndex: pageNum - 1,
              x: item.transform[4],
              y: item.transform[5],
      ...
      if (positions.signature) break; // Found it, stop scanning pages
    }

    pdf.destroy();
  } catch (err) {
    console.error('[scanPdfForSignatureMarkers] Failed:', err);
    // Return empty - caller will fall back to page mode
  }
...
/**
 * White-out signature marker text in the unsigned PDF and replace with
 * a subtle "Sign here" indicator so the signer sees a clean document
 */
async function prepareUnsignedPdf(
  pdfBytes: Uint8Array,
  positions: SignaturePositions
): Promise<Uint8Array> {
  if (!positions.signature) return pdfBytes;
  ...
  try {
    ...
    const page = pages[positions.signature.pageIndex];
    ...
    // White-out the "Signature: ____..." text
    page.drawRectangle({
      x: sigPos.x - 2,
      y: sigPos.y - 5,
      width: (sigPos.width || 200) + 4,
      height: (sigPos.height || 20) + 10,
      color: rgb(1, 1, 1),
    });

    // Draw subtle "Sign here" indicator
    ...
    // White-out and replace date portion if detected
    if (positions.date) {
      const datePos = positions.date;
      page.drawRectangle({
        x: datePos.x - 2,
        y: datePos.y - 5,
        width: (datePos.width || 150) + 4,
        height: (datePos.height || 20) + 10,
        color: rgb(1, 1, 1),
      });
      page.drawText('[ Date ]', {
        x: datePos.x + 10,
    ...
    console.error('[prepareUnsignedPdf] Failed to prepare PDF:', err);
...
serve(async (req) => {
  ...
  try {
    ...
    // Scan PDF for signature placement markers (e.g., "Signature: ____")
    console.log('[send-for-signature] Scanning PDF for signature markers...');
    let signaturePositions: SignaturePositions = {};
    let processedPdfBytes = pdfBytes;

    try {
      signaturePositions = await scanPdfForSignatureMarkers(pdfBytes);

      if (signaturePositions.signature) {
        console.log('[send-for-signature] Found signature marker at:', JSON.stringify(signaturePositions.signature));
        // White-out marker text and add "Sign here" indicator in the unsigned PDF
        processedPdfBytes = await prepareUnsignedPdf(pdfBytes, signaturePositions);
      } else {
        console.log('[send-for-signature] No signature markers found, will use page mode');
      }
    } catch (scanError) {
      console.error('[send-for-signature] PDF scanning failed (non-blocking):', scanError);
      // Fall back to original PDF and page mode
    }

    // Compute SHA-256 hash of the final unsigned PDF for tamper detection
    const unsignedPdfHash = await computePdfHash(processedPdfBytes);
    console.log('[send-for-signature] PDF hash computed:', unsignedPdfHash.substring(0, 16) + '...');
    ...
    const { data: signingToken, error: tokenError } = await supabaseAdmin
      ...
      .insert({
        ...
        signature_positions: Object.keys(signaturePositions).length > 0 ? signaturePositions : null,
        // Tamper detection
