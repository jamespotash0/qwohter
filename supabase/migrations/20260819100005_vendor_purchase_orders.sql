...
-- manufacturer, because that is how the money and the paperwork actually move:
-- a 1,200-line job might be six POs to six factories, each acknowledged,
-- shipped, and invoiced on its own schedule.
--
--   sales_order -> order_lines
--                     |
--                     +--> vendor_po (per vendor) -> po_lines -> order_line
--
-- Decisions baked in:
--
--   * po_lines reference order_lines rather than duplicating them. The order
--     line is the thing that was sold; a PO line is a claim on some of its
--     quantity. Partial ordering is normal -- a line can be split across two
--     POs when a customer releases a job in phases.
--
--   * Quantities ordered are NOT stored on order_lines. Issuing a PO writes an
--     'ordered' event and order_line_fulfillment sums it, same as every other
--     fulfillment quantity.
--
--   * Every PO line carries BOTH the price ordered at and the price the
...
CREATE TABLE IF NOT EXISTS public.vendor_pos (
  ...
  status text NOT NULL DEFAULT 'Draft'
    CHECK (status IN (
      'Draft',        -- assembled, not yet sent
      'Sent',         -- transmitted to the vendor
      'Acknowledged', -- vendor confirmed, possibly with changes
      'Partially Received',
      'Received',
      'Cancelled',
      'Closed'
  ...
  -- Vendor terms snapshotted at issue time. A vendor's standing terms can
  -- change; what this PO was placed under must not.
  payment_terms text,
  freight_terms text,

  -- Where it went, and how. Snapshotted for the same reason.
  sent_at timestamptz,
  sent_to_email text,
...
CREATE INDEX IF NOT EXISTS idx_vendor_pos_vendor
  ON public.vendor_pos (vendor_id, status);
CREATE INDEX IF NOT EXISTS idx_vendor_pos_org_status
  ON public.vendor_pos (organization_id, status);
-- PO numbers are assigned on issue, so uniqueness applies only once set.
...
CREATE TABLE IF NOT EXISTS public.po_lines (
  ...
  -- manufacturer must not be deletable out from under its purchase order.
...
BEGIN
  ...
  INSERT INTO public.vendor_pos (
    organization_id, sales_order_id, vendor_id, po_number, status,
  ...
  -- A line is only 'Ordered' once its FULL quantity is on a purchase order.
...
COMMENT ON FUNCTION public.create_vendor_po_with_lines(jsonb, jsonb) IS
  'Atomically create a vendor PO, its lines, and the ordered fulfillment events. SECURITY INVOKER, so RLS applies. Pricing is read from order_lines, not trusted from the caller.';
...
-- ============================================================================
-- What a manufacturer came back with, against what was ordered. This is the
-- screen a PM opens in the morning: acknowledgments where the price or the ship
-- date moved. security_invoker so the caller's RLS applies.
...
COMMENT ON VIEW public.po_line_variance IS
  'Acknowledged purchase order lines against what was ordered. variance_status is awaiting_ack | price | date | price_and_date | match.';
